#!/usr/bin/env python3
"""Should MATCH FORECASTS use the blend instead of last season alone? OOS on 2025.
...
OUT = os.path.join(REPO, "data", "forecast_blend_2025.json")
...
def main():
    ...
    variants = ["V0_margin", "PRIOR_only", "V4_mix25"]
    ...
    for frac in CHECKPOINTS:
        ...
        for v, wh in (("V0_margin", 0.0), ("PRIOR_only", None),
                      ("V4_mix25", 0.25)):
            sc = dict(prior_z) if wh is None else blend_mix(wh)
            scored = []
            for m in future:
    ...
    print("\npaired bootstrap (2000x), delta vs PRIOR_only, pooled:")
    verdicts = {}
    for v in ("V0_margin", "V4_mix25"):
        deltas = []
        for _ in range(2000):
            tot0 = tot1 = 0.0
            for ci in range(len(paired_all["V0_margin"])):
                base = paired_all["PRIOR_only"][ci]
