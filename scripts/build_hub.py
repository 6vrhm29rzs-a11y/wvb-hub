...
.mrow .mline{display:flex;gap:3px;justify-content:flex-end;min-width:0}
.mset{display:flex;flex-direction:column;gap:3px;padding:2px 5px;border-radius:3px;background:var(--cs-cell,rgba(255,255,255,.03))}
.mset b{font:600 12.5px/1.15 var(--mono);color:var(--slate);font-variant-numeric:tabular-nums;text-align:center;min-width:17px}
.mset b.sw{color:var(--chalk)}
.mline.islive .mset.now{background:color-mix(in oklab,var(--coral) 18%,transparent)}
.mline.islive .mset.now b{color:var(--coral)}
...
@media (max-width:560px){
  ...
  /* ⚠ THE LINE SCORE IS THE FIRST THING TO GO ON A PHONE. Five sets of
     two numerals cannot share a 390px row with two school names; the
     set tally and the names are what matter at that width, and the
     per-set detail is one tap away in the match itself. */
  .mrow{grid-template-columns:60px minmax(0,1fr) auto;gap:8px}
  .mrow .mline{display:none}
  /* the venue is a tap away in the match itself; at 390px the names
     and the result are the whole job */
...
/* THE SET-BY-SET LINE SCORE -- the detail a volleyball scoreboard is FOR.
   The row already knew who played and how many sets each won; the ~1,000px
   between the names and the score was empty, and the only place the per-set
   points existed was a collapsed card grid further down the page.

   ⚠ TWO SOURCES, AND THEY DO NOT OVERLAP THE WAY YOU EXPECT. The crawled
   ledger carries `sets` on a final and is authoritative. The live feed carries
   `sets` WHILE A MATCH IS BEING PLAYED and then EMPTIES the array the moment
   it goes final -- so a match that has just ended has line scores in neither
   place until the next crawl catches up. That is the same seam that once made
   a finished match vanish from the Scores tab entirely. Here it degrades
   correctly: no sets means no strip, never a placeholder and never a zero.

   ⚠ DO NOT VALIDATE THESE AGAINST 25. Set one of SMU-Penn St. read 24-22 and
   that is a REAL completed set: the exhibition is played first-two-to-21. A
   plausibility rule invented from the standard format would have suppressed a
   true score, which is the R5 failure in reverse.

   ⚠ A LIVE SET IS PROVISIONAL AND IS MARKED AS ONE. Live figures get revised,
   downward included, so the set in progress is rendered as in progress rather
   than as a result. Nothing here is accumulated or stored. */
/* ⚠ NOT `setStrip` -- THAT NAME WAS ALREADY TAKEN, and taking it again would
   have silently emptied the two cards a reader looks at first. `setStrip(sets,
   live)` (line ~9136) is called by the live band and the just-finished cards
   with a bare array; a second declaration of the same name in the same scope
   simply wins, so those calls would have arrived here as (m=Array, live=false,
   st=undefined), found no `.sets` property on an array, and returned an empty
   strip. No error, no warning -- the set scores would just have stopped
   appearing on the Today tab. Same family as the duplicate `id="sbody"`. */
function rowSetStrip(m, live, st) {
  const raw = (live && live.sets && live.sets.length) ? live.sets
    : (m.sets && m.sets.length ? m.sets : null);
  if (!raw) return '<span class="mline"></span>';
  const playing = st === 'live';
  /* the in-progress set is the last one, and only while the match is live */
  const cells = raw.map((sv, i) => {
    const a = sv[0], h = sv[1];
    if (a === null || a === undefined || h === null || h === undefined) return '';
    const now = playing && i === raw.length - 1;
    return '<span class="mset' + (now ? ' now' : '') + '"' +
      ' title="set ' + (i + 1) + (now ? ' (in progress)' : '') + '">' +
      '<b class="' + (!now && +a > +h ? 'sw' : '') + '">' + esc(String(a)) + '</b>' +
      '<b class="' + (!now && +h > +a ? 'sw' : '') + '">' + esc(String(h)) + '</b>' +
      '</span>';
  }).join('');
  return '<span class="mline' + (playing ? ' islive' : '') + '">' + cells + '</span>';
}

/* WHERE IT WAS PLAYED, AND WHAT IT WAS PART OF -- reported, never inferred.
   The row had ~830px of empty space between the school names and the score,
   and the payload was already carrying the event and the venue for every
   fixture. This is the honest way to spend it: real fields, not decoration.

   ⚠ VENUE IS THE ONE FIELD THIS PROJECT HAS ALREADY GOT WRONG BY GUESSING.
   The dashboard once inferred "at <home team>" and was wrong the first weekend
   it mattered -- Kentucky-Wisconsin and Louisville-Texas A&M were both at
