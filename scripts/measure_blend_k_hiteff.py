#!/usr/bin/env python3
"""How fast should the blend lean into this season, WITH the hit-eff channel? OOS on 2025.
...
OUT = os.path.join(REPO, "data", "forecast_blend_k_2025.json")
...
def main():
    ...
    variants = ["PRIOR_only"] + ["k%g" % kk for kk in KS]
    ...
    for frac in CHECKPOINTS:
        ...
        for v, wh in [("PRIOR_only", None)] + [("k%g" % kk, kk) for kk in KS]:
            sc = dict(prior_z) if wh is None else blend_mix(0.25, wh)
            scored = []
            for m in future:
        ...
        print("  %4.0f%% (%.1f matches/team): " % (frac * 100, 2.0 * cut / max(1, len(prior_z))) + "  " + "  ".join(
            "%s %.4f" % (v, pooled[v][-1]) for v in variants)) if False else None
    ...
    print("\npaired bootstrap (2000x), delta vs k13.5 (shipped), pooled:")
    verdicts = {}
    for v in ("k7", "k10"):
        deltas = []
        for _ in range(2000):
            tot0 = tot1 = 0.0
            for ci in range(len(paired_all["k13.5"])):
                base = paired_all["k13.5"][ci]
                cand = paired_all[v][ci]
                n = min(len(base), len(cand))
                idx = [rng.randrange(n) for _ in range(n)]
                tot0 += RF.auc([base[i] for i in idx])
                tot1 += RF.auc([cand[i] for i in idx])
            deltas.append((tot1 - tot0) / len(paired_all["k13.5"]))
        deltas.sort()
